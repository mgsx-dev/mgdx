...
import net.mgsx.gdx.graphics.GL32.DebugProc;
...
public class GL32DebugControlSketch extends ScreenAdapter
{
	...
	public static class GLDebug {
		...
		private static class DebugReader {
			...
			public void fetchAndLog(){
				for(;;){
					int count = Mgdx.gl32.glGetDebugMessageLog(n, sources, types, ids, severities, lengths, messageLog);
					if(count == 0) break;
		 ...
		 * @param callback when null, messages can be obtained using {@link DebugReader#fetch()}
		 */
		public static void setCallback(DebugProc callback){
			Mgdx.gl32.glDebugMessageCallback(callback);
		}
		...
		private static void insertMessage(int source, int type, int id, int severity, String message){
			...
			Mgdx.gl32.glDebugMessageInsert(source, type, id, severity, message);
		}
		...
		public static void enable(boolean enabled, int source, int type, int severity){
			Mgdx.gl32.glDebugMessageControl(source, type, severity, null, enabled);
		}
		...
		public static void enableIDs(boolean enabled, int source, int type, int...ids){
			...
			Mgdx.gl32.glDebugMessageControl(
				source,
				type, 
				GL32.GL_DONT_CARE, 
				idsBuffer, 
		...
		public static void log(int source, int type, int id, int severity, String message){
			...
			Gdx.app.log("GLDebug", "source:" + strSource + " type:" + strType + " id:" + id + " severity:" + strSeverity + " message:" + message);
		}
	...
	public GL32DebugControlSketch() {
		...
		// Labeling
		Mgdx.gl32.glObjectLabel(GL20.GL_TEXTURE, texture.getTextureObjectHandle(), "myTexture");
		String label = Mgdx.gl32.glGetObjectLabel(GL20.GL_TEXTURE, texture.getTextureObjectHandle());
	...
	public void render(float delta) {
		...
		Mgdx.gl32.glPushDebugGroup(GL32.GL_DEBUG_SOURCE_APPLICATION, 57, "sprite batch drawing 1");
		batch.begin();
		batch.draw(texture, 0, 0, 1, 1);
		batch.end();
		Mgdx.gl32.glPopDebugGroup();
		
		Mgdx.gl32.glPushDebugGroup(GL32.GL_DEBUG_SOURCE_APPLICATION, 57, "sprite batch drawing 2");
		batch.begin();
		batch.draw(texture, 0, 0, 1, 1);
		batch.end();
		Mgdx.gl32.glPopDebugGroup();
