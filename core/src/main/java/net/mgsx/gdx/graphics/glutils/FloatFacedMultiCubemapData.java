...
public class FloatFacedMultiCubemapData implements CubemapData
{
	 ...
	 */
	public FloatFacedMultiCubemapData(int width, int height, GLFormat format, FloatBuffer[] allData, int levels)
	{
		this.levels = levels;
		...
		for(int level = 0 ; level<levels ; level++){
			for(int face = 0 ; face < 6 ; face++){
				...
				FlexTextureData td = new FlexTextureData(width, height, level, format.internalFormat, format.format, format.type, allData[index]);
				data[index] = td;
			}
			width/=2;
			height/=2;
