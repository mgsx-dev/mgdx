...
layout(location = COLOR_LOCATION) out vec4 out_color;
#endif
...
#ifdef textureFlag
in MED vec2 v_texCoord0;
#endif // textureFlag

#ifdef textureCoord1Flag
in MED vec2 v_texCoord1;
...
#ifdef brdfLUTTexture
	vec2 brdf = texture(u_brdfLUT, vec2(pbrSurface.NdotV, 1.0 - pbrSurface.perceptualRoughness)).xy;
...
void main() {
    ...
    out_color = vec4(pow(color,vec3(1.0/GAMMA_CORRECTION)), baseColor.a);
#else
    out_color = vec4(color, baseColor.a);
#endif
	// Blending and Alpha Test
#ifdef blendedFlag
	out_color.a = baseColor.a * v_opacity;
#ifdef alphaTestFlag
...
void main() {
	...
	float ao = 1.0;
#ifdef occlusionTextureFlag
    ao = mix(ao, texture(u_OcclusionSampler, v_occlusionUV).r, u_OcclusionStrength);
#endif
...
#endif
	out_color.rgb = mix(out_color.rgb, u_fogColor.rgb, fog * u_fogColor.a);
#endif
